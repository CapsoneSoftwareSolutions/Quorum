...
def user_following_topics(usr_pk):
	...
	if follows:
		for f in follows:
			...
			str_r+=str_tp
	else:str_r="There is no followed topics..."
	return str_r
...
def user_favourites(usr_pk):
	...
	if favs:
		for f in favs:
			...
			str_r+=str_fv
	else:str_r='There is no favourite questions...'
	return str_r
@register.simple_tag	
def user_postedquestions(usr_pk):
	print usr_pk
	usr=User.objects.get(id=int(usr_pk))
	ques=Question.objects.filter(user=usr)
	str_r=""
	if ques:
		for f in ques:
			str_pos="<a href='/quorum/question/%s/'>%s</a><br>" %(f.pk,f.question)
			str_r+=str_pos
	else:str_r='There is no posted questions...'
	return str_r	